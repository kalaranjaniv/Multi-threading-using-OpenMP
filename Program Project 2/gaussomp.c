...
void main(int argc, char **argv) {
  ...
  A=(double**) malloc(MAXN*sizeof(double));
   for(j=0;j<MAXN;j++)
{
A[j]=(double *) malloc(MAXN * sizeof(double));
}
B=(float*)malloc(MAXN*sizeof(float));
X=(float*)malloc(MAXN*sizeof(float));
         ...
         for(row=norm+1;row<N;row++)
          {
           ...
           #pragma parallel for schedule(static) num_threads(procs)
